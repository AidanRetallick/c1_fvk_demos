...
namespace Parameters
{
  ...
  //  unsigned Problem_case = ; // Nonaxisymmetric_shear_buckling; //
  //  Axisymmetric_shear_buckling;
  ...
  double Y_intersect =
    sqrt(B1 * B1 * (1.0 - X_intersect * X_intersect / (A1 * A1)));
  ...
  Vector<CurvilineGeomObject*> Parametric_curve_pt = {
    &Upper_parametric_elliptical_curve, &Lower_parametric_elliptical_curve};
  ...
  : Element_area(element_area)
{
  ...
  for (unsigned i_bound = 0; i_bound < n_bound; i_bound++)
  {
    ...
    {
      oomph_info << "Node " << i_b_node << " is on boundary " << i_bound
                 << std::endl;
    }
  }
...
void UnstructuredFvKProblem<ELEMENT>::build_mesh()
{
  ...
  Outer_curvilinear_boundary_pt[0] =
    new TriangleMeshCurviLine(Parameters::Parametric_curve_pt[0],
                              zeta_start,
                              zeta_end,
                              nsegment,
                              Outer_boundary0);
  ...
  Outer_curvilinear_boundary_pt[1] =
    new TriangleMeshCurviLine(Parameters::Parametric_curve_pt[1],
                              zeta_start,
                              zeta_end,
                              nsegment,
                              Outer_boundary1);
  ...
  for (unsigned i_node = 0; i_node < n_node; i_node++)
  {
    ...
    cout << "Node " << i_node << " at (" << node_pt->x(0) << ","
         << node_pt->x(1) << ") is on " << tot << " boundaries: ";
...
// template<class ELEMENT>
// void
// UnstructuredFvKProblem<ELEMENT>::pin_all_displacements_and_rotation_at_centre_node()
// {
...
int main(int argc, char** argv)
{
  ...
  UnstructuredFvKProblem<FvKelement> problem(element_area);
