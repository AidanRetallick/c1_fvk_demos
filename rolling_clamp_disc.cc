...
//     {
//       // Call the generic residuals function with flag set to 0 hierher why
//       no arg?
...
//     void
//     fill_in_point_force_and_torque_contribution_to_residuals(Vector<double>
//     &residuals)
//     {
//       // No further action
...
//     /// Local coordinates of point at which point force_and_torque is
//     applied Vector<double> S_point_force_and_torque;
...
namespace Parameters
{
  ...
  // Get the unit function for applying simple inhomogenous BCs
...
private:
  ...
  /// Pin the in plane displacements (dofs 0-1) at the centre
  void pin_in_plane_displacements_and_rotation_at_centre_node();
...
void UnstructuredFvKProblem<ELEMENT>::build_mesh()
{
  ...
  zeta_start = Pi;
  zeta_end = 2.0 * Pi;
  nsegment = (int)(Pi / sqrt(Element_area));
...
//==start_of_pin_in_plane_displacements_and_rotation_at_centre_node===
/// pin all displacements and rotations in the centre
//====================================================================
template<class ELEMENT>
void UnstructuredFvKProblem<
  ELEMENT>::pin_in_plane_displacements_and_rotation_at_centre_node()
{
  // Choose non-centre node on which we'll supress
  // the rigid body rotation around the z axis.
  double max_x_potentially_pinned_node = -DBL_MAX;
...
{
  // Pin the centre in place (not w)
  pin_in_plane_displacements_and_rotation_at_centre_node();
  ...
  for (unsigned b = 0; b < nbound; b++)
  {
    ...
    for (unsigned e = 0; e < nb_element; e++)
    {
      ...
      // A rolling clamp, so we set all w dofs except the second
...
//==============================================================================
/// A function that upgrades straight sided elements to be curved. This
/// involves
// Setting up the parametric boundary, F(s) and the first derivative F'(s)
// We also need to set the edge number of the upgraded element and the
// positions of the nodes j and k (defined below) and set which edge (k) is to
// be exterior
/*            @ k */
/*           /( */
/*          /.
  ...
  const unsigned& ibound)
{
  ...
  for (unsigned e = 0; e < n_els; e++)
  {
    ...
    // hierher: ouch! This seems to map (x,y) to zeta! This is at best
    // possible to within a tolerance. Needs a redesign!
    ...
    {
      throw OomphLibError(
        "Decreasing parametric coordinate. Parametric coordinate must "
        "increase as the edge is traversed anti-clockwise.",
        OOMPH_CURRENT_FUNCTION,
        OOMPH_EXCEPTION_LOCATION);
    } // end checks
...
/// dofs. For example if we know w(n,t) = f(t) (where n and t are the normal
/// and tangent to a boundary) we ALSO know dw/dt and d2w/dt2. NB no rotation
/// is needed if the edges are completely free!
//======================================================================
template<class ELEMENT>
  ...
  for (unsigned e = 0; e < n_element; e++)
  {
    ...
    for (unsigned b = 0; b < n_bound; b++)
    {
      ...
      if (!boundary_node.empty())
      {
        ...
        el_pt->rotated_boundary_helper_pt()->add_nodal_boundary_parametrisation(
...
int main(int argc, char** argv)
{
  ...
  // Do an initial solve and docinfo
  problem.newton_solve();
  problem.doc_solution();

  // Loop over the steps and solve
